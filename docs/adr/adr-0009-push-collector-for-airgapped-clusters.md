...
ADR-0005 adopted a **central pull** topology: a single longue-vue process holds
N kubeconfigs and pulls every cluster from outside. This works when longue-vue
...
- **Production clusters behind strict egress firewalls** that block all
  ...
  longue-vue to that list violates the principle of least privilege.
- **Multi-tenant environments** where the CMDB operator (who runs longue-vue)
  cannot obtain a kubeconfig for a tenant's cluster; only the tenant's own
  automation can push data out.

In all three cases the pull model fails: longue-vue cannot reach the Kubernetes
...
observations to longue-vue over the REST API**.
...
Ship a **push-mode collector** (`longue-vue-collector`) that runs inside
an air-gapped cluster, polls the local Kubernetes API, and pushes
observations to a remote longue-vue instance over HTTPS.

### Binary

Extract the existing `internal/collector` package into a standalone
binary under `cmd/longue-vue-collector/`. The binary reuses the same
`KubeSource` implementation (in-cluster or kubeconfig) and the same
ingestion logic (`ingestNodes`, `ingestNamespaces`, …) but writes
through an **HTTP client targeting the longue-vue REST API** instead of
the direct `Store` interface.

The `longue-vue` binary keeps its embedded pull-mode collectors unchanged.
...
The push collector authenticates to longue-vue with a **bearer token**
...
- **Path prefix rewrite**: if the gateway exposes longue-vue under a
  sub-path (e.g. `/longue-vue/v1/…` instead of `/v1/…`), the collector
  supports `LONGUE_VUE_SERVER_URL=https://gw:443/longue-vue` and prepends the
...
- **mTLS to the gateway**: when the gateway requires client-certificate
  ...
  LONGUE_VUE_CLIENT_KEY=/etc/longue-vue/tls/client.key
  ```
...
- Egress network policy allowing HTTPS to the longue-vue endpoint only.
...
- **POS-002**: Each push collector only holds credentials for its own
  cluster (in-cluster ServiceAccount) + one longue-vue PAT. A compromised
...
- **POS-005**: The `longue-vue-collector` binary is a minimal, static Go
...
- **NEG-001**: Second binary to build, release, and version. Mitigated
  ...
  once, consumed by both `longue-vue` and `longue-vue-collector`.
...
- **NEG-004**: The push collector must trust longue-vue's (or the gateway's)
...
- **NEG-005**: When a gateway sits between the collector and longue-vue,
  error diagnostics become harder — a 503 may come from the gateway,
  not longue-vue. The `apiclient` must log the full response status and
...
- **Description**: The air-gapped collector writes a JSON snapshot to a
  ...
  longue-vue side, which imports it via a `POST /v1/import` endpoint.
...
- **Description**: The push collector opens a gRPC stream to longue-vue and
...
- **Description**: Instead of a push collector, establish a VPN or SSH
  tunnel from longue-vue to the air-gapped cluster's API server.
...
- **IMP-001**: Create `cmd/longue-vue-collector/main.go`. Parse env vars,
  build a `KubeClient`, build an `apiclient.Store`, build a
  `collector.Collector`, call `Run(ctx)`. Signal handling identical to
  longue-vue (SIGINT/SIGTERM → context cancel → graceful drain).
...
- **IMP-004**: Add a `Dockerfile.collector` (or a build stage in the
  existing Dockerfile) producing the `longue-vue-collector` static binary.
...
- **IMP-006**: Integration test: start longue-vue with a test PG, start
  `longue-vue-collector` pointed at longue-vue's HTTP listener with a
...
- **IMP-008**: Add an integration test with a test HTTP proxy between
  the collector and longue-vue to validate proxy/gateway transparency
