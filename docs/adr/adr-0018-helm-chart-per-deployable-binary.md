...
Argos ships **four** deployable binaries today:

| Binary             | Role                                                                                             | Packaging today                                          |
|--------------------|--------------------------------------------------------------------------------------------------|----------------------------------------------------------|
| `argosd`           | Central server: API, UI, store, in-process pull collector goroutines                             | Helm chart `charts/argos/` (chart 0.14.0 / app 0.11.1)   |
| `argos-ingest-gw`  | DMZ reverse-proxy (ADR-0016) — fronts argosd's mTLS-only ingest listener                         | Helm chart `charts/argos-ingest-gw/` (chart 0.1.0)       |
| `argos-collector`  | Push-mode Kubernetes collector for air-gapped / DMZ-isolated clusters (ADR-0009)                 | **Raw Kustomize manifests** under `deploy/collector/`    |
| `argos-vm-collector` | Push-mode VM collector for cloud-provider VMs (ADR-0015 §IMP-003); one instance per cloud account | **Raw Kustomize manifests** under `deploy/vm-collector/` |

The two missing charts cause concrete operator pain:

1. **Inconsistent install UX.** A SecNumCloud operator running argosd via Helm must drop down to `kubectl apply -k deploy/collector/` for the collector. Two different upgrade procedures, two different secret-management stories, two different conventions for image override.
...
4. **Inconsistent observability wiring.** `charts/argos-ingest-gw/` ships an optional `ServiceMonitor` + `PrometheusRule`; the collector and vm-collector ship neither — operators write their own.
5. **mTLS to the DMZ gateway is undocumented in the deploy manifests.** ADR-0016 §7 introduced the gateway as the new collector ingress, but `deploy/collector/deployment.yaml` still bakes in the legacy "talk straight to argosd" shape. The chart is the natural place to expose `argosTLS.existingSecret` + `argosTLS.caSecret` + `argosTLS.extraHeaders` as first-class values.
...
We add **two new independent Helm charts**, mirroring the layout and conventions of `charts/argos-ingest-gw/`:

- `charts/argos-collector/` — packages the `argos-collector` push-mode Kubernetes collector binary.
- `charts/argos-vm-collector/` — packages the `argos-vm-collector` push-mode VM collector binary.

We keep `charts/argos/` (argosd) and `charts/argos-ingest-gw/` (gateway) unchanged. We deprecate `deploy/collector/` and `deploy/vm-collector/` raw manifests in a follow-up release; for ADR-0018 itself, we keep them as a fallback and add a pointer in `deploy/README.md` to the new charts.

### 1. Independent charts, not subcharts

Each chart is a top-level Helm chart, **not** a subchart of `charts/argos/`. Reasons:

- The collector typically runs in a *target* cluster — different namespace, often a different physical cluster, from argosd. An umbrella subchart relationship forces co-installation, breaking the multi-cluster topology of ADR-0005.
- The vm-collector deploys **one Helm release per cloud account** (ADR-0015 §IMP-004). N subchart instances inside a single argosd release is ergonomically broken — you can't `helm upgrade` one account in isolation.
...
Each new chart follows the same skeleton as `charts/argos-ingest-gw/`:
...
Both charts expose the same shape for the parts that match. The keys mirror existing argosd / gateway conventions so operators reuse muscle memory.

| Key                            | Default                                  | Purpose                                                           |
|--------------------------------|------------------------------------------|-------------------------------------------------------------------|
| `image.repository`             | `ghcr.io/sthalbert/argos-<chart-name>`   | Image registry override for air-gap mirrors                       |
...
```
helm install argos-vm-collector-<account> charts/argos-vm-collector \
  --namespace argos-vm-collectors --create-namespace \
  --set accountName=<account> --set region=<region> \
  --set tokenSecret.existingSecret=argos-vm-collector-<account>-pat
```

### 6. Secret handling

Neither chart generates tokens or PATs. Both expect the operator to create the secret out-of-band (typically via `kubectl create secret generic` or via Vault Agent). This matches the `charts/argos/` bootstrap pattern (the admin password is operator-supplied) and avoids putting plaintext PATs in Helm release state, where they would persist in `helm get values` output forever.
...
|-------------------------|-----------------------|--------------------|
| `argos-collector`       | `0.1.0`               | `0.11.1`           |
| `argos-vm-collector`    | `0.1.0`               | `0.11.1`           |
...
The four chart versions evolve independently. Releasing a new argosd version does **not** force a chart bump on the collector charts unless their bundled defaults need to change.
...
The `deploy/collector/deployment.airgap.yaml` profile is reproducible via the chart by setting `proxy.httpsProxy=...` and `argosTLS.caSecret=...`; the README documents this 1:1 mapping.

### 9. Helm linting in CI

Phase 3 adds `helm lint charts/argos-collector charts/argos-vm-collector` and `helm template ...` smoke tests to the existing `make check` flow (or the GitHub Actions `ci.yml` if `make check` doesn't already run helm). The smoke test asserts:
...
- **Multi-account vm-collector becomes ergonomic.** `helm install argos-vm-collector-<account> ... --set accountName=<account>` per account.
...
- The Docker images for both binaries already exist (`Dockerfile.collector`, `Dockerfile.vm-collector` build to `ghcr.io/sthalbert/argos-collector:<tag>` and `argos-vm-collector:<tag>`). The charts simply reference them. No changes to the build pipeline.
...
1. **Subcharts under `charts/argos/`.** Rejected: forces co-installation with argosd, which breaks the multi-cluster (ADR-0005) and one-instance-per-cloud-account (ADR-0015) topologies. Also pollutes the umbrella chart's release history with binary-specific bumps.
...
4. **Generate the chart from a CRD or operator.** Out of scope. Argos is not an operator and adopting one for packaging is disproportionate to the problem.

## References

- ADR-0005 — Multi-cluster collector topology (`charts/argos-collector/` deploys per-cluster, per ADR-0005).
- ADR-0008 — SecNumCloud chapter 8 asset management (audit-grade deployment artifacts).
- ADR-0009 — Push collector for air-gapped clusters (the `argos-collector` binary).
- ADR-0015 — VM collector for non-Kubernetes platform VMs (the `argos-vm-collector` binary; one instance per cloud account).
- ADR-0016 — DMZ ingest gateway (precedent for an independent chart per binary; mTLS posture inherited by both new charts).
- ADR-0017 — Public listener TLS posture (the `argosTLS.*` values surface mirrors ADR-0017's `argosd.tls.*` shape).
