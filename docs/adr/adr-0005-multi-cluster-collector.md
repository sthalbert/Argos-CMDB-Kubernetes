...
- The longue-vue data model already carries `cluster_id` on every entity, and the FK chain already scopes namespaces / pods / workloads / services / ingresses under their parent cluster. The *schema* is multi-cluster-ready. Only the *collector* isn't.

This ADR decides how one or more Kubernetes clusters are polled into a single longue-vue CMDB.

**Two axes force the decision:**

1. **Process topology** — one longue-vue polling N clusters from outside, or N longue-vue instances each polling one cluster from inside?
...
- longue-vue is a single Go binary with a static build (Dockerfile PR). It can run in-cluster with a mounted kubeconfig or a ServiceAccount with cluster-scoped RBAC.

## Decision

**Adopt the "one longue-vue, N internal collectors" (central pull) topology as the v1 multi-cluster shape**, with the push model explicitly kept as a future option the data model already supports.

**Process topology:** a single longue-vue process runs a `Collector` goroutine per target cluster. Each goroutine owns its own `KubeClient` (kubeconfig + clientset), its own ticker, its own per-tick `poll()` cycle. All write through the shared `Store`. All anchor on the root `signal.NotifyContext` so SIGINT / SIGTERM drains every collector before the HTTP server shuts down.
...
- `kubeconfig` is a filesystem path. An empty string means "use in-cluster config" (longue-vue's own ServiceAccount) — useful when longue-vue is deployed inside one of the target clusters.
...
- **POS-004**: The push model (agent-per-cluster) stays available as a future option via the existing REST API + scoped tokens. When longue-vue needs horizontal scaling, or when network policy forbids outbound connections from a central longue-vue to every cluster, that path opens without redesigning data storage.
...
- **ALT-001**: **Description**: Run a longue-vue instance (or a slimmer collector-only binary) inside each cluster, using its in-cluster ServiceAccount. Each agent POSTs observations to a central longue-vue via the REST API with a scoped bearer token.
- **ALT-002**: **Rejection Reason for v1**: Requires a second deployment target (the agent), a token-distribution story, and network reachability *to* the central longue-vue from each cluster. The central-pull model is a smaller step with the same data model. The push model is explicitly kept on the roadmap — ADR-0001 already committed to API-based pushes being supported, and this ADR doesn't foreclose it.

### Sidecar / separate collector binary

- **ALT-003**: **Description**: Split the collector out of `longue-vue` into its own binary (e.g., `longue-vue-collector`), deploy it independently.
- **ALT-004**: **Rejection Reason**: Adds a second binary, a second Dockerfile, a second release cadence, before there's a demonstrated need. The `internal/collector` package can already run on its own inside longue-vue; if a future deployment requires a standalone binary, extracting the package is a small refactor.

### Filesystem-based config directory

- **ALT-005**: **Description**: Point at `/etc/longue-vue/clusters/` and treat every `*.yaml` file as one kubeconfig; the filename (or the kubeconfig's `current-context`) becomes the cluster name.
...
- **IMP-001**: Add `LONGUE_VUE_COLLECTOR_CLUSTERS` parsing in `cmd/longue-vue/main.go`: accept JSON, validate non-empty `name`, unique names, non-negative count, fall through to `LONGUE_VUE_CLUSTER_NAME` / `LONGUE_VUE_KUBECONFIG` when the new var is absent.
...
- **IMP-005**: Tests: extend `cmd/longue-vue` / `internal/collector` coverage with:
