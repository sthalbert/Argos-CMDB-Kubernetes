...
> **Production note:** the `deploy/` manifests are a quickstart example, not the supported production surface. Per [ADR-0018](../adr/adr-0018-helm-charts.md), Helm is the recommended deployment path. See [Deploy with Helm](helm.md) for a one-command install with optional bundled PostgreSQL.
