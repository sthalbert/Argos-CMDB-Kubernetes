...
func TestAuthCache_LRUEvicts(t *testing.T) {
	...
	c.Put("aaa", "full-aaa", &Caller{Name: "a"})
	c.Put("bbb", "full-bbb", &Caller{Name: "b"})
	c.Put("ccc", "full-ccc", &Caller{Name: "c"}) // should evict "aaa"
	if c.Len() != 2 {
...
func TestAuthCache_LRURecency(t *testing.T) {
	...
	c.Put("ccc", "full-ccc", &Caller{Name: "c"})
	if _, ok := c.Get("bbb", "full-bbb"); ok {
...
func TestAuthCache_WrongFullToken(t *testing.T) {
	t.Parallel()
	c := NewAuthCache(10, time.Minute)
	c.Put("aaa", "full-aaa-correct", &Caller{Name: "a"})
...
func TestAuthCache_ConcurrentAccess(t *testing.T) {
	...
	for range 64 {
		...
		go func() {
			...
			prefix := "pref"
			full := "full-token"
			c.Put(prefix, full, &Caller{Name: "concurrent"})
			c.Get(prefix, full)
			c.Invalidate(prefix)
			c.Len()
		}()
...
func TestAuthCache_RevocationSubscriber(t *testing.T) {
	t.Parallel()
	c := NewAuthCache(10, time.Minute)
	c.Put("tok", "full-tok", &Caller{Name: "test"})
